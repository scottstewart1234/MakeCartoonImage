...
def make_cartoon_image2(ImagePath, #input image path
                  cartoonPath, #output image path
                  AdaptiveThreshold1=25, #The lower this is the more lines appear
                  AdaptiveThreshold2=9, #The higher this is the less lines appear. must be less than AdaptiveThreshold1
                  ThickerLinesIterations = 3, #The higher this is the thicker lines will be
                  RemoveDotsIterations = 15,  #The higher this is the thinner lines will be. Removes some noisy dots as well
    ...
    for b in range(buckets):
       ...
       maximum = int(round(amax/buckets*(b+1)))
       try:
           set_value = int(round(np.average(h, weights=((grayScaleImage >= minimum) & (grayScaleImage < maximum)))))
           set_values = int(round(np.average(s, weights=((grayScaleImage >= minimum) & (grayScaleImage < maximum)))))
           set_valuev = int(round(np.average(v, weights=((grayScaleImage >= minimum) & (grayScaleImage < maximum)))))
       except:
           #There is nothing in this bin.
           set_value = 0
       
       h_new = np.where((grayScaleImage >= minimum) & (grayScaleImage < maximum), set_value, h)
       s_new = np.where((grayScaleImage >= minimum) & (grayScaleImage < maximum), set_values, s)
       v_new = np.where((grayScaleImage >= minimum) & (grayScaleImage < maximum), set_valuev, v)
    hsv[:,:,0] = h_new
    hsv[:,:,1] = s_new
    hsv[:,:,2] = v_new
    #Convert back to bgr
